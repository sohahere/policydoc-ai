...
REPORT_FILE_PATH = PROJECT_ROOT / "evaluation" / "hard_reranked_retrieval_report.csv"
COLLECTION_NAME = "hard_reranked_policy_documents"
...
def index_hard_pdf():
    ...
    ids = [
        f"hard_reranked_chunk_{chunk.metadata['chunk_id']}"
        for chunk in chunks
    ]
...
def retrieve_candidates(collection, embedding_model, question: str):
...
def evaluate_reranked_retrieval():
    print("\nHARD PDF RERANKED RETRIEVAL EVALUATION")
    print("=" * 100)
    ...
    print(f"Candidate-k from ChromaDB: {CANDIDATE_K}")
    ...
    for item in eval_questions:
        ...
        rows.append(
            {
                ...
                "reranked_pages": str(retrieved_pages),
                "reranked_chunk_ids": str(retrieved_chunk_ids),
                "reranked_content_types": str(retrieved_content_types),
                "rerank_scores": str(rerank_scores),
                f"hit_at_{RERANK_TOP_K}": hit,
        ...
        print(f"Reranked chunk IDs: {retrieved_chunk_ids}")
        print(f"Reranked content types: {retrieved_content_types}")
        print(f"Rerank scores: {rerank_scores}")
        print(f"Hit@{RERANK_TOP_K}: {hit}")
    ...
    with open(REPORT_FILE_PATH, "w", newline="", encoding="utf-8") as file:
        fieldnames = [
            ...
            "reranked_pages",
            "reranked_chunk_ids",
            "reranked_content_types",
            "rerank_scores",
            f"hit_at_{RERANK_TOP_K}",
            "reciprocal_rank",
        ]
    ...
    print("\nFINAL HARD RERANKED RETRIEVAL METRICS")
    print("=" * 100)
    print(f"Recall@{RERANK_TOP_K}: {recall_at_k:.2f}")
    print(f"MRR: {mrr:.2f}")
